...
const Header: FC<IProps> = ({
  ...
  return (
    <header
      className={`${style.header} flex flex-col text-white relative w-full justify-start mx-auto container-spacing`}
    >
      <div
        className={`${style.header_container} flex items-center justify-between my-5`}
      >
        <div
          className={`${style.header_container_logo} flex gap-x-3 items-center md:items-center md:flex-row md:gap-x-3`}
        >
      ...
      {showNav && (
        <div
          style={{ zIndex: 99 }}
          className={`${style.transitionOpacity} ${showNav ? style.transitionOpacity_active : ""} fixed top-0 right-0 w-64 min-h-screen pt-14 px-10 bg-[#2F4F4F] text-[rgb(234,179,8)] flex flex-col items-center gap-5 overflow-x-hidden`}
        >
          <button
